  ...
  <div className="absolute inset-0 bg-transparent opacity-20 animate-aurora" />
        ...
        <main className="relative z-10 min-h-screen pt-28 pb-20 px-6 lg:px-20">
          ...
          <section className="text-center max-w-5xl mx-auto mb-20">
            <h1 className="text-6xl font-extrabold mb-6 bg-white text-transparent bg-clip-text drop-shadow-lg">
          ...
          <section className="grid grid-cols-1 md:grid-cols-3 gap-10 max-w-6xl mx-auto mb-28">
            {principles.map((val, idx) => (
              <div
                key={idx}
                className="group relative rounded-3xl p-10 border-2 border-cyan-400 bg-white/10 backdrop-blur-xl shadow-[0_10px_60px_rgba(0,255,255,0.3)] transform transition hover:scale-[1.06] hover:-rotate-x-2 hover:rotate-y-2"
          ...
          <section className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto text-center mb-20">
            {metrics.map((item, i) => (
              ...
              >
                <div className="text-4xl text-cyan-300 mb-3">{item.icon}</div>
          ...
          {/* CTA Section */}
          <section className="relative bg-gradient-to-r from-cyan-500 to-blue-700 rounded-3xl shadow-[0_0_60px_rgba(0,255,255,0.5)] p-12 max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 transform hover:scale-[1.02] transition-all">
            <div>
              <h2 className="text-4xl font-extrabold mb-3">Begin Your Journey with Poeage</h2>
