...
export default function Ai() {
  return (
    <>
      <Header />
      <div className="pt-28 md:pt-32 pb-20 px-6 sm:px-10 md:px-20 min-h-screen bg-gradient-to-br from-sky-50 via-cyan-100 to-white relative">
        {/* Grid Lines */}
        ...
        {/* Feature Grid */}
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-10 relative z-10">
          {features.map((feature, index) => (
            <Link
              to={`/enquiry?topic=${encodeURIComponent(feature.title)}`}
              key={index}
            >
              <div className="relative group rounded-3xl border border-gray-200 bg-white/60 backdrop-blur-xl p-6 transition-all duration-500 hover:shadow-2xl hover:scale-[1.03] cursor-pointer">
                <div className="absolute -inset-0.5 rounded-3xl bg-gradient-to-br from-cyan-400 to-blue-500 opacity-0 group-hover:opacity-100 blur-lg transition duration-500 z-0" />
